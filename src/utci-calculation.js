...
/**
 * Calculates the Universal Thermal Climate Index (UTCI) using the standard
 * polynomial approximation from meteorological inputs.
 *
 * @param {number} Ta - Air temperature in °C.
 * @param {number} [Tmrt=Ta] - Mean radiant temperature in °C. Falls back to `Ta` when omitted.
 * @param {number} va - Wind speed in m/s.
 * @param {number} RH - Relative humidity in %.
 * @returns {number} UTCI approximation in °C.
 */
calcUTCI = function (Ta, Tmrt = Ta, va, RH) {
